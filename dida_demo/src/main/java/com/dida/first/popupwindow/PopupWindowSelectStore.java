 ...
 * 
 */
public class PopupWindowSelectStore extends
		PopupWindowBaseDown<List<BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity>> {
	private MyListView mylv_market_detail_select;
	private MyAdapter myAdapter;
	private TextView tv_param_select_count;
	private int count = 1;

	public PopupWindowSelectStore(View parentView, Activity activity,
								  List<BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity> data) {
	...
	class MyAdapter extends MyBaseListViewAdapter<BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity> {
		public MyAdapter(List<BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity> list) {
			super(list);
		...
		public View getItemView(final int position, View convertView,
			...
			List<BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity.AttrValuesEntity> attrValues = list.get(position).getAttrValues();
			tv_param_slecet_name.setText(paramName);
			for (final BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity.AttrValuesEntity attr : attrValues) {
		...
		public void setFalse(int position) {
			List<BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity.AttrValuesEntity> attrValues = list.get(position).getAttrValues();
			for (BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity.AttrValuesEntity  attr : attrValues) {
				attr.setCheck(false);
			}
			notifyDataSetChanged();
		}
		...
		public String getParams() {
			...
			for (BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity outVallues : list) {
				List<BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity.AttrValuesEntity> attrValues = outVallues.getAttrValues();
				for (BeanDetailPingouStore.ResEntity.PurchaseAttrsEntity.AttrValuesEntity attr : attrValues) {
					if (attr.isCheck()) {
