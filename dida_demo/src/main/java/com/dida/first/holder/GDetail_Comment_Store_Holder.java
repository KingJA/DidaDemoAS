...
import com.dida.first.adapter.PingouCommentInStoreAdapter;
 ...
 *
 */
public class GDetail_Comment_Store_Holder extends BaseHolder<BeanDetailPingouStore> implements View.OnClickListener{
	...
	public GDetail_Comment_Store_Holder(Activity activity){
	...
	public void refreshView() {
		...
		List<BeanDetailPingouStore.ResEntity.ComGroupDetailEntity.ReplysEntity> replys = getData().getRes().getComGroupDetail().getReplys();
		...
		if (replys.size()>0){
			BeanDetailPingouStore.ResEntity.ComGroupDetailEntity.ReplysEntity latestComment = replys.get(0);
			...
			if (latestComment.getSubReplys().size()>0){
				...
				List<BeanDetailPingouStore.ResEntity.ComGroupDetailEntity.ReplysEntity.SubReplysEntity> subReplys = latestComment.getSubReplys();
				PingouCommentInStoreAdapter pingouCommentInUserAdapter = new PingouCommentInStoreAdapter(subReplys, activity);
